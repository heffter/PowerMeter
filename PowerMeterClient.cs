...
namespace PowerMeterClient
{
  ...
  public class PowerMeterClient
  {
    ...
    public PowerReading GetCurrentPower()
    {
      try
      {
        var response = _httpClient.GetAsync($"{_baseUrl}/api/current").GetAwaiter().GetResult();
        response.EnsureSuccessStatusCode();
    ...
    public ServerStatus GetStatus()
    {
      try
      {
        var response = _httpClient.GetAsync($"{_baseUrl}/api/status").GetAwaiter().GetResult();
        response.EnsureSuccessStatusCode();
  ...
  //  {
  //    PowerMeterClient client = new PowerMeterClient("http://localhost:5000");
